...
public sealed class ProfileMatcher
{
    ...
    public MatchResult Match(IReadOnlyList<MonitorIdentity> monitors, IReadOnlyList<WallpaperProfile> profiles)
    {
        if (monitors.Count == 0 || profiles.Count == 0)
            return new MatchResult { Status = MatchStatus.NoMatch, Message = "No monitor profiles are available" };
        ...
        if (candidates.Count == 0)
            return new MatchResult { Status = MatchStatus.NoMatch, Message = $"There are {monitors.Count} monitor(s), but no matching profile" };
        ...
        if (best.Score <= 0)
            return new MatchResult { Status = MatchStatus.NoMatch, Profile = best.Profile, Score = best.Score, RunnerUpScore = runnerUp, Message = "There is not enough identity evidence" };
        ...
        if (duplicateNoSerial && (assignmentTie || best.WeakOnly))
            return BuildAmbiguous(best, runnerUp, "Identical monitors without reliable serials require manual confirmation");
        if (tied || assignmentTie)
            return BuildAmbiguous(best, runnerUp, "Monitor mapping scores are too close; manual confirmation is required");
        ...
        return new MatchResult
        {
            ...
            Message = compatible ? "Matched using compatible fingerprints" : "Matched using stable identity",
