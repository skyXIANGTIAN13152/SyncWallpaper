...
独立宿主保持安全进程边界。阶段 2 已在该边界内实现 TaskbarHost v3（副屏条、过滤、按屏归属、应用分组、逐窗口动作、固定项、DWM 缩略图、时钟、底边自动隐藏、单副屏 AppBar 与关闭清理），但仍不注入 Explorer；多副屏工作区在 Shell 无法可靠逐屏预留时安全回退。Shell、ScreenSaver、Remote 和 Online 仍主要是生命周期骨架。后续功能必须继续留在独立进程中，不能把 Hook/网络/第三方提供器放回核心宿主。
...
当前自动化测试覆盖模块模式、启动/停止幂等、依赖顺序、非法循环、超时、故障隔离、一次退避恢复、crash-loop 持久化禁用、IPC JSON 往返/乱码拒绝、显示/音频事务回滚、关闭验证和独立宿主真实启动/退出；TaskbarHost v3 另有窗口过滤、分组、固定项存储、缩略图布局、混合 DPI、自动隐藏、工作区策略、真实三屏窗口和资源门禁。真实 Explorer 重启、Win10、通知区镜像和高风险 Hook 仍需后续确认。
